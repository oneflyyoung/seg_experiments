...
def fcn(split):
    n = caffe.NetSpec()
    pydata_params = dict(split=split, mean=(0, 0, 0),
            seed=1337)
    if split == 'train':
    ...
    n.score_pool3 = L.Convolution(n.pool3, num_output=2, kernel_size=1, pad=0,
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
    ...
    n.loss = L.SoftmaxWithLoss(n.score, n.label,
            loss_param=dict(normalize=True, ignore_label=100))
