...
class VOCSegDataLayer(caffe.Layer):
    ...
    def load_image(self, idx):
        ...
        """
        im = Image.open('{}/JPEGImages/{}.png'.format(self.voc_dir, idx))
    ...
    def load_label(self, idx):
        ...
        The leading singleton dimension is required by the loss.
        """
        im = Image.open('{}/seg/{}.png'.format(self.voc_dir, idx))
        label = np.array(im, dtype=np.uint8)
        label = label[np.newaxis, ...]
        return label
...
class SBDDSegDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        # load indices for images and labels
        split_f  = '{}/imagesets/{}.txt'.format(self.sbdd_dir,
                self.split)
        self.indices = open(split_f, 'r').read().splitlines()
        self.idx = 0
    ...
    def load_label(self, idx):
        ...
        The leading singleton dimension is required by the loss.
        """
        im = Image.open('{}/seg/{}.png'.format(self.sbdd_dir, idx))
        label = np.array(im, dtype=np.uint8)
        label = label[np.newaxis, ...]
        return label
